...
func getOp(ctx antlr.ParserRuleContext, i int) int {
	// In binary rules (e.g. expr (OP expr)*), the operator for the i-th RHS (where i starts at 1)
	// is located between lhs (i-1) and rhs (i).
	// Child 0 = expr
	// Child 1 = op
	// Child 2 = expr
	// Child 3 = op ...
	// So operator for rhs[i] is at index 2*i - 1.
	// Since loop 'i' starts at 1 (for the first RHS), this formula is correct.
...
func (g *Generator) VisitEqualityExpression(ctx *parser.EqualityExpressionContext) interface{} {
	...
	// Iterate through all RHS operands
		...
		if op.LPAREN() != nil {
			...
			if op.ArgumentList() != nil {
				for _, arg := range op.ArgumentList().AllArgument() {
					if v := g.Visit(arg.Expression()); v != nil { args = append(args, v.(ir.Value)) }
				}
			}
			...
			if ptr, ok := curr.Type().(*types.PointerType); ok {
				if st, ok := ptr.ElementType.(*types.StructType); ok {
					if idx, ok := g.analysis.StructIndices[st.Name][fieldName]; ok {
						currPtr = g.ctx.Builder.CreateStructGEP(st, curr, idx, "")
...
func (g *Generator) VisitLiteral(ctx *parser.LiteralContext) interface{} {
	...
	if ctx.CHAR_LITERAL() != nil {
		if len(txt) >= 2 {
			r := []rune(txt)[1] // Simple extraction, real parser handles escapes
...
func (g *Generator) VisitIntrinsicExpression(ctx *parser.IntrinsicExpressionContext) interface{} {
	if ctx.SIZEOF() != nil { return g.ctx.Builder.CreateSizeOf(g.resolveType(ctx.Type_()), "") }
	if ctx.ALIGNOF() != nil { return g.ctx.Builder.CreateAlignOf(g.resolveType(ctx.Type_()), "") }
	if ctx.BIT_CAST() != nil {
	...
	if ctx.MEMSET() != nil { return g.ctx.Builder.CreateMemSet(args[0], args[1], args[2]) }
	if ctx.MEMCPY() != nil { return g.ctx.Builder.CreateMemCpy(args[0], args[1], args[2]) }
	if ctx.MEMMOVE() != nil { return g.ctx.Builder.CreateMemMove(args[0], args[1], args[2]) }
	if ctx.STRLEN() != nil { return g.ctx.Builder.CreateStrLen(args[0], "") }
	...
	if ctx.VA_ARG() != nil { return g.ctx.Builder.CreateVaArg(args[0], g.resolveType(ctx.Type_()), "") }
	if ctx.VA_END() != nil { g.ctx.Builder.CreateVaEnd(args[0]); return g.getZeroValue(types.Void) }
	
	if ctx.RAISE() != nil { g.ctx.Builder.CreateRaise(args[0]) }
