...
func (tg *TypeGenerator) GenType(t ast.TypeRef) types.Type {
	...
	case *ast.ExternType:
		// Extern types are already IR-level; just recursively translate.
		return tg.genExternTypeRef(typ)
	}

	return types.Void
}

// GenExternType translates an extern type AST node (not a full TypeRef) to an
// IR type. Used for extern function parameter and return types.
...
// genExternTypeRef handles the ast.ExternType variant which appears as
// TypeRef in extern declarations.
func (tg *TypeGenerator) genExternTypeRef(t *ast.ExternType) types.Type {
	// ExternType is not a standard TypeRef; it's generated by the translator
	// for extern blocks. Fall through to void for now — the actual type info
	// was captured in the ExternFunc.Params slice as plain TypeRef nodes.
	return types.Void
}

// parseInt is a small helper used for array-length literals.
func parseInt(s string) (int64, error) {
	// strconv is imported in expr.go; replicate here to avoid import cycles.
	var v int64
	if len(s) > 2 && s[:2] == "0x" {
		for _, ch := range s[2:] {
			v <<= 4
