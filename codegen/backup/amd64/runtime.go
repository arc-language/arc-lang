...
func (r *Runtime) EmitAsyncTaskCreate(fn *ir.Function, stackSize int) {
	// 1. Save Callee-Saved Regs
	r.asm.Push(R12)

	// 2. Allocate AsyncResult
	r.asm.Mov(RegOp(RDI), ImmOp(0), 64)
	r.asm.Mov(RegOp(RSI), ImmOp(16), 64)
	r.asm.Mov(RegOp(RDX), ImmOp(layout.ProtRead|layout.ProtWrite), 64)
	r.asm.Mov(RegOp(R10), ImmOp(layout.MapPrivate|layout.MapAnonymous), 64)
	r.asm.Mov(RegOp(R8), ImmOp(-1), 64)
	r.asm.Xor(RegOp(R9), RegOp(R9))
	...
	r.asm.Mov(NewMem(RAX, 0), ImmOp(0), 64) 
	r.asm.Mov(NewMem(RAX, 8), ImmOp(0), 64) 
	r.asm.Mov(RegOp(R12), RegOp(RAX), 64)
	...
	r.asm.Push(RAX) // Save Stack Base
	...
	r.asm.Mov(RegOp(RDI), NewMem(RSP, 0), 64) 
	...
	// --- PARENT ---
	r.asm.Pop(RCX) 
	r.asm.Mov(RegOp(RAX), RegOp(R12), 64) 
	r.asm.Pop(R12) 
	doneLabel := r.asm.JmpRel(0)

	// --- CHILD ---
	r.asm.PatchJump(childLabel)
	r.asm.And(RegOp(RSP), ImmOp(-16)) // Align
