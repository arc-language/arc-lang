...
func (c *compiler) compileAsyncTaskCreate(inst *ir.AsyncTaskCreateInst) error {
	...
	// This generates the clone3 syscall, allocates the stack, and returns the result handle in RAX.
	// We use a default stack reservation of 64KB (which grows via the signal handler).
	c.runtime.EmitAsyncTaskCreate(inst.Callee, 64*1024)
